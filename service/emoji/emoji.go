...
func init() {
	err := json.Unmarshal(emojiFile, &Emojis)
	if err != nil {
		log.Panic(err)
	}
	InitEmojiSlice()
...
func InitEmojiSlice() []Emoji {
	for _, emoji := range Emojis {
		EmojiSlice = append(EmojiSlice, emoji)
	}
	return EmojiSlice
}
...
func LookupEmoji(emojiString string) (emoji Emoji, err error) {
	...
	hexKey := helper.StringToHexKey(emojiString)
...
func FindAll(input string) (detectedEmojis SearchResults) {
	...
	for index, r := range runes {
		...
		hexKey := helper.RunesToHexKey([]rune{r})
		...
		for {
			...
			if len(potentialMatches) == 1 {
				...
				previousKey = hexKey
				hexKey = hexKey + "-" + helper.RunesToHexKey([]rune{runes[nextIndex]})
